# ==============================================================================
# COMBINED ROBUSTNESS CHECK SCRIPT - WITH EXPLICIT INTERACTION TERMS
#
# This script performs two analyses as a robustness check:
# 1. Third Down Conversion: Models the outcome of third-down plays.
# 2. Fourth Down Attempt: Models the decision to attempt a fourth-down conversion.
#
# Key modifications:
# - Filters data to only include plays with > 5 minutes (300 seconds) remaining
# - Creates explicit columns for all interaction and polynomial terms
# - Extracts marginal effects directly without dealing with formula interactions
#
# All results will be saved in a directory named 'robustness_checks_5min'.
# ==============================================================================
...
if (!dir.exists("robustness_checks_5min")) {
  dir.create("robustness_checks_5min")
}

cat("--- Libraries Loaded and 'robustness_checks_5min' directory created. ---\n\n")
...
# ROBUSTNESS CHECK: Filter for plays with more than 300 seconds remaining
third_down_data <- third_down_data[seconds_remaining_in_half > 300]
cat("Third down data filtered for > 300 seconds remaining.\n")
...
# Save all results
write.csv(tval_matrix_3rd, "robustness_checks_5min/third_down_t_values.csv")
write.csv(se_matrix_3rd, "robustness_checks_5min/third_down_standard_errors.csv")
write.csv(coef_matrix_3rd, "robustness_checks_5min/third_down_coefficients_marginal_effects.csv")
write.csv(pval_matrix_3rd, "robustness_checks_5min/third_down_p_values.csv")
...
write.csv(joint_test_results_3rd, "robustness_checks_5min/third_down_joint_significance.csv")
...
# ROBUSTNESS CHECK: Filter for plays with more than 300 seconds remaining
attempt_data <- attempt_data[seconds_remaining_in_half > 300]
cat("Fourth down data filtered for > 300 seconds remaining.\n")
...
# Save all results
write.csv(tval_matrix_4th, "robustness_checks_5min/fourth_down_attempt_t_values.csv")
write.csv(se_matrix_4th, "robustness_checks_5min/fourth_down_attempt_standard_errors.csv")
write.csv(coef_matrix_4th, "robustness_checks_5min/fourth_down_attempt_coefficients_marginal_effects.csv")
write.csv(pval_matrix_4th, "robustness_checks_5min/fourth_down_attempt_p_values.csv")
...
write.csv(joint_test_results_4th, "robustness_checks_5min/fourth_down_joint_significance.csv")
cat("--- Fourth down joint significance tests saved ---\n\n")

# Summary of results
cat("\n=== ANALYSIS COMPLETE ===\n")
cat("The following files have been saved in 'robustness_checks_5min':\n")
cat("\nThird Down Analysis:\n")
cat("  - third_down_t_values.csv\n")
cat("  - third_down_standard_errors.csv\n")
cat("  - third_down_coefficients_marginal_effects.csv\n")
cat("  - third_down_p_values.csv\n")
cat("  - third_down_joint_significance.csv\n")
cat("\nFourth Down Analysis:\n")
cat("  - fourth_down_attempt_t_values.csv\n")
cat("  - fourth_down_attempt_standard_errors.csv\n")
cat("  - fourth_down_attempt_coefficients_marginal_effects.csv\n")
cat("  - fourth_down_attempt_p_values.csv\n")
cat("  - fourth_down_joint_significance.csv\n")
