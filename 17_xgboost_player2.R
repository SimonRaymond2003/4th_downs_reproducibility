## 1. Setup
# Number of bootstrap samples per hyperparameter setting
n_bootstrap_per_param <- 250
n_bootstrap <- 1000
# 1. Define the hyperparameter grid
hyper_grid <- expand.grid(
  nrounds = seq(100, 3000, by = 100)
)
...
## 2. Load Data
# ==============================================================================
# MODIFIED: Removed the tryCatch block for direct data loading.
...
# MODIFICATION: Define predictors for XGBoost.
# As per the request, coach_, starter_, and missing_information variables are EXCLUDED from the XGBoost model.
# # These variables will still be used in the second-stage LPM, Logit, and Probit models.
# cat("\n--- MODIFICATION: Excluding coach_, starter_, and missing_information_ variables from the first-stage XGBoost model. ---\n")
# coach_vars_to_remove_xgb <- grep("^coach_", names(selection_data), value = TRUE)
...
# xgb_vars_to_exclude <- c("attempt", "my_id", coach_vars_to_remove_xgb, player_vars_to_remove_xgb, missing_vars_to_remove_xgb)
# predictor_vars <- setdiff(names(selection_data), xgb_vars_to_exclude)
# cat(sprintf("%d Coach variables removed from XGBoost model predictors.\n", length(coach_vars_to_remove_xgb)))
# cat(sprintf("%d Player variables removed from XGBoost model predictors.\n", length(player_vars_to_remove_xgb)))
# cat(sprintf("%d Missing information variables removed from XGBoost model predictors.\n", length(missing_vars_to_remove_xgb)))
# cat(sprintf("Total variables removed: %d\n", length(coach_vars_to_remove_xgb) + length(player_vars_to_remove_xgb) + length(missing_vars_to_remove_xgb)))
# These variables will still be used in the second-stage LPM, Logit, and Probit models.
...
player_vars_to_remove_xgb <- grep("^starter_", names(selection_data), value = TRUE)
missing_vars_to_remove_xgb <- grep("^missing_information", names(selection_data), value = TRUE)
xgb_vars_to_exclude <- c("attempt", "my_id", player_vars_to_remove_xgb, missing_vars_to_remove_xgb)
predictor_vars <- setdiff(names(selection_data), xgb_vars_to_exclude)
cat(sprintf("Coach variables retained in XGBoost model predictors.\n"))
cat(sprintf("%d Player variables removed from XGBoost model predictors.\n", length(player_vars_to_remove_xgb)))
cat(sprintf("%d Missing information variables removed from XGBoost model predictors.\n", length(missing_vars_to_remove_xgb)))
cat(sprintf("Total variables removed: %d\n", length(player_vars_to_remove_xgb) + length(missing_vars_to_remove_xgb)))
# Impute NAs with 0
...
# --- MODIFIED: Hyperparameter Tuning with Bootstrap Validation ---
cat("\n--- Tuning XGBoost: Bootstrap Validation for Optimal Number of Rounds ---\n")



# Define the fixed XGBoost parameters
xgb_params <- list(
...
# Initialize a data.table to store the tuning results
tuning_results <- data.table()


# 2. Loop through the grid to perform bootstrap validation for each nrounds setting
cat(sprintf("Starting grid search across %d settings with %d bootstrap samples each...\n", 
            nrow(hyper_grid), n_bootstrap_per_param))

for (i in 1:nrow(hyper_grid)) {
  nrounds_to_test <- hyper_grid$nrounds[i]
  cat(sprintf("\nTesting setting %d/%d: nrounds = %d\n", i, nrow(hyper_grid), nrounds_to_test))
  
  # Store AUCs for this hyperparameter setting
  bootstrap_aucs_for_param <- numeric(n_bootstrap_per_param)
  
  # Inner loop: Bootstrap validation
  for (b in 1:n_bootstrap_per_param) {
    cat(sprintf("\r  Bootstrap sample %d/%d", b, n_bootstrap_per_param))
    
    # Create bootstrap sample indices
    train_idx <- unique(sample(1:n_total, size = n_total, replace = TRUE))
    test_idx <- setdiff(1:n_total, train_idx) # Out-of-bag samples
    
    # Skip if no test samples (rare but possible)
    if(length(test_idx) == 0) {
      bootstrap_aucs_for_param[b] <- NA
      next
    }
    ...
    if (!is.na(auc_val)) {
      bootstrap_aucs_for_param[b] <- auc_val
    } else {
      bootstrap_aucs_for_param[b] <- NA
    }
  }
  
  # Calculate mean AUC for this hyperparameter setting (removing NAs)
  valid_aucs <- bootstrap_aucs_for_param[!is.na(bootstrap_aucs_for_param)]
  mean_auc <- mean(valid_aucs)
  sd_auc <- sd(valid_aucs)
  
  cat(sprintf("\n  Mean AUC: %.5f (SD: %.5f, based on %d valid samples)\n", 
              mean_auc, sd_auc, length(valid_aucs)))
  
  # Store the results
  results_row <- data.table(
    n.trees = nrounds_to_test,
    mean_auc = mean_auc,
    sd_auc = sd_auc,
    n_valid_samples = length(valid_aucs)
  )
  tuning_results <- rbind(tuning_results, results_row)
}

# 3. Find the best hyperparameter setting
best_params <- tuning_results[which.max(mean_auc)]
best_nrounds <- best_params$n.trees

cat(sprintf("\n--- Bootstrap Grid Search Complete ---\n"))
cat(sprintf("Optimal number of rounds (trees) found: %d (with Mean Bootstrap AUC: %.5f, SD: %.5f)\n", 
            best_nrounds, best_params$mean_auc, best_params$sd_auc))

# Save the detailed tuning results
fwrite(tuning_results, "script_outputs_players/xgboost_bootstrap_tuning_results_player.csv")

# 4. Plot the tuning results to see the AUC curve with error bars
tuning_plot <- ggplot(tuning_results, aes(x = n.trees, y = mean_auc)) +
  ...
  geom_vline(xintercept = best_nrounds, linetype = "dashed", color = "red", size = 1) +
  labs(
    title = "XGBoost Tuning: Bootstrap Validation AUC vs. Number of Rounds",
    subtitle = paste("Optimal nrounds =", best_nrounds, "at the red dashed line (250 bootstrap samples per setting)"),
    x = "Number of Rounds (Trees)",
    y = "Mean Bootstrap AUC (± 1 SD)"
  ) +
  theme_minimal()
ggsave("script_outputs_players/xgboost_bootstrap_tuning_plot_auc_player.png", plot = tuning_plot, width = 10, height = 6)

# Save all best hyperparameters to a CSV file
best_hyperparameters <- data.table(
  parameter = c("nrounds", "eta", "max_depth", "min_child_weight", 
                "subsample", "colsample_bytree", "objective", "eval_metric",
                "mean_bootstrap_auc", "sd_bootstrap_auc"),
  value = c(best_nrounds, 
            xgb_params$eta,
            ...
            best_params$mean_auc,
            best_params$sd_auc)
)

# Write the hyperparameters to CSV
...
cat("\n--- Assessing Performance on 100 Bootstrap Samples ---\n")
...
# ==============================================================================
# Calculate Inverse Mills Ratio and its polynomial forms
...
cat(sprintf("- %d bootstrap samples per hyperparameter setting\n", n_bootstrap_per_param))
cat(sprintf("- Best nrounds: %d with mean bootstrap AUC: %.5f\n", best_nrounds, best_params$mean_auc))
